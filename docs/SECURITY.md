...
Legend: ✅ implemented · ♾️ structural (mitigated, not eliminated). In the section 3
matrix, the mitigation columns read: ● total prevention · ◐ partial · — not addressed
by that layer.
...
That covers LLM agents (the original motivation), but it equally covers malicious or
compromised npm/pip/cargo packages, VS Code extensions, and build scripts — any code
that executes or writes files in those containers. The design doesn't assume the agent
is well-behaved; it assumes hostile code is already running there, and limits what that
buys an attacker.
...
connect to each other's unix sockets, and read and modify each other's files. So we
don't try to separate trusted from untrusted code *inside* one container; we keep
anything powerful in a *different* container. The isolation boundary is the container —
namespaces plus mounts — not the uid.

---

## 2. Adoption layers

The toolkit is adopted in three composable layers. Each builds on the previous, and
...
`docker-compose.yml`. One container; if you run an agent it runs *alongside* the dev,
same uid. This adds:

- **VS Code host-channel hardening** (section 5) — blanks and scrubs the env channels
  that leak host credentials, and reaps the askpass and `code`-CLI IPC sockets.
- **OS-level container isolation** (section 6) — `cap_drop: ALL`, `no-new-privileges`,
  no sudo, bridge networking, no Docker socket, `ptrace_scope ≥ 1`.
...
is driven over a shared-volume `tmux` socket (section 9). This adds (section 8):

- The agent can't read the dev's at-rest secrets (`~/.ssh`, `~/.aws`, home) — they're
  unmounted — nor scrape the dev's process memory or env (separate namespaces).
...
`/creds` shelf; giving the agent a *narrower* grant than the dev is the broker variant
([CREDENTIAL-BROKER.md](./CREDENTIAL-BROKER.md)). This adds (sections 10–11):
...
Two kinds of credential show up below, and it's worth keeping them distinct: a
consumer's **own at-rest secrets** (personal keys or tokens it holds directly, e.g. an
SSH signing key — risk #1) versus **vended credentials** (the short-lived scoped tokens
the sidecar provides — risks #10–13). Isolation (L2) keeps the dev's *own* secrets away
from the agent; vending (L3) governs the *vended* tokens.

| # | Risk vector | L1 harden | L2 isolate | L3 vend | Residual |
|---|---|:--:|:--:|:--:|---|
| 1 | Untrusted code reads a consumer's **own at-rest secrets** (e.g. the dev's `~` — *not* vended creds) | — | ● *(agent)* | — | deps/extensions in the *dev* container still read the dev's own secrets — no layer stops that |
| 2 | **Cross-process scraping** (`/proc/<pid>/environ`, ptrace heap → Settings-Sync/Copilot tokens) | ◐ | ● *(agent)* | — | L1 `ptrace_scope` blocks heap; same-uid env still readable *within* a container |
...
| 9 | **Extension-host RCE → host escape** (write files + window reload) | ◐ | — | — | ♾️ **not closed** — needs headless / web-client (section 7) |
...
| 11 | **Use + exfiltration of a *granted* credential** | — | — | ◐ | **not prevented** — accepted; scope+TTL limit damage, pair with egress allowlisting |
...
| 14 | **Shared `/workspace` poisoning** (`.git/hooks` runs as the dev) | — | ◐ | — | discipline: `core.hooksPath` for agent-touched repos (section 8) |
| 15 | **Applying agent output in a trusted context** (your machine / CI) | — | — | — | **not mitigated** — discipline only: review diffs first (section 12) |
| 16 | **Over-broad credential scope** | — | — | ◐ | **operator's call** — vending enforces *scoped*, not *which* scope |
| 17 | **Host / machine compromise** | — | — | — | **out of scope** — the host is the real perimeter (section 4) |
...
Trust is concentric: each tier holds only what it must, and powerful credentials live
as far from untrusted code as we can put them.

| Tier | Holds | Trust |
|------|-------|-------|
| **Consumer containers** (dev, agent) | only short-lived, scoped, *vended* credentials | **untrusted** |
...
Two boundaries carry the weight: sidecar ↔ consumer (the credential architecture,
section 10) and consumer ↔ host (the VS Code channel and RCE surface of a
desktop-attached dev container, sections 5 and 7).
...
credentials and capabilities into it. Each is a channel that untrusted code *in that
container* — an extension, a dependency's child process — could ride:

| Channel | Exposes | Control | Status |
|---|---|---|---|
| **git askpass / OAuth** (`GIT_ASKPASS`, `VSCODE_GIT_IPC_HANDLE` socket) | the human's VS Code GitHub **OAuth token** (`repo`+`workflow`) once the *git* session is authorized | `git.useIntegratedAskPass:false` + `remoteEnv` blanks vars; the socket is **reaped** (below) | ✅ ♾️ |
| **host-credential-proxy** (injected `credential.helper`) | the host's *stored* git creds | `remote.containers.gitCredentialHelperConfigLocation:none` | ✅ |
...
**Two-layer env neutralization (you need both).** `remoteEnv` blanks the vars for
processes VS Code *spawns*, which covers untrusted code spawned non-interactively (an
extension's child processes, a `bash -c`). But VS Code re-injects
...
(`/etc/profile.d/50-scrub-vscode-git-auth.sh` plus a `/etc/bash.bashrc` include, shipped
in `base`) is what cleans those, and it matters because anything launched from an
integrated terminal inherits its env. `remoteEnv` covers spawned processes, the scrub
covers interactive terminals, and you need both.

**The IPC-socket residual (♾️) and the reaper.** The git and CLI extensions stay
enabled, so VS Code keeps recreating `vscode-git-*.sock` (askpass/OAuth) and
`vscode-ipc-*.sock` (the `code` CLI). These sockets have no caller authentication — any
same-uid process can connect, regardless of the blanked env var. A container entrypoint
...
race. The real guarantee is upstream: do not authorize the VS Code git OAuth session,
since with no session the socket vends nothing. Settings Sync and Copilot don't create
it; only an explicit git sign-in or "Publish to GitHub" does.
...
`ptrace_scope ≥ 1`. Never run with `ptrace_scope=0`, `--privileged`, or `CAP_SYS_PTRACE`
— any of those re-opens heap scraping.

---

## 6. Layer 1 · Container isolation (OS-level) ✅

- **Bridge networking** (no `network_mode`/`ipc: host`) — no host net or IPC namespace;
  this also closes the forwarded-SSH-agent SOCKS/LAN-pivot vector.
...
- **No Docker socket** — a writable `docker.sock` is root-equivalent on the host and
  voids every boundary. (Container-to-container *networking* and shared volumes remain,
  so you don't need it to drive the agent — section 9.)
...
The deepest risk this architecture can't fully close: a workspace extension runs in the
dev container's extension host and can call host-only VS Code commands over the
client↔server RPC bridge — `workbench.action.terminal.newLocal` + `sendSequence` gives
arbitrary shell on the desktop. Untrusted code in the dev container (a malicious
extension, or a dependency's postinstall) doesn't even need to *be* an extension — it
can write files: poison an installed extension's JS
(`~/.vscode-server/extensions/.../*.js`, same-uid-writable) or coerce
`remote.extensionKind` via the bind-mounted `.vscode/settings.json` /
`devcontainer.json`. On the next window reload, the poisoned code runs in the ext host
with host reach, bypassing all credential isolation.

It's inherent to the bridge model. The real mitigations are architectural: the web
client (`code serve-web`), or attaching no trusted desktop client at all. Until then
it's documented, not mitigated — and `.vscode`/`.devcontainer` are writable by code in
the dev container, with some settings applying on reload, so review their diffs before
*reloading*, not just before rebuilding (section 12).
...
installs, builds, and extensions still run in the dev container — handled by credential
scoping and discipline, not isolation (section 11).
...
*(Credential vending — the sidecar and `/creds` shelf — is Layer 3, section 10. Layer 2
is just these two containers sharing the `/workspace` code mount; it works with or
without vending.)*

**Why a separate container, not a second uid in one.** Two uids in one container would
need a privileged launcher to `setuid()` down — but `cap_drop: ALL` strips `CAP_SETUID`
even from root, so nothing inside the hardened container can switch uid, and adding the
cap back weakens the best hardening you've got. A separate container lets the runtime
set `user:` at creation (outside the container's caps): no in-container privilege, no
bespoke launcher, `cap_drop: ALL` intact, and separate PID/IPC namespaces (so the agent
can't see, `ptrace`, or read `/proc/environ` of the dev's processes).

**Same uid, isolated by mounts.** Both run as uid 1000. Across containers, isolation
comes from namespaces and mount topology, not the uid:

> **Invariant: the agent container mounts only `/workspace` + its broker socket —
> never `~/.ssh`, `~/.aws`, `~/.claude`, or anything else.**

The dev's keys, creds, and home live in the *dev* container's filesystem; unmounted,
they're unreachable regardless of uid. Same uid keeps it simple (agent-written files
are 1000-owned, the dev commits cleanly — no shared-group or setgid dance). Two notes:
...
1. Credentials come from the shared read-only `/creds` shelf — the same for both
   ...
   *different* credential policy than the dev is the broker variant
...
2. The one residual: the agent can write anything in shared `/workspace`, including
   `.git/hooks/*`, which would execute *as the dev* on the dev's next git op. Mitigate
   with discipline (`git config core.hooksPath /dev/null` for agent-touched repos), not
   uid.
...
are — so the dev reaches the isolated agent without `docker exec`. Run a tmux server in
the agent container on a shared-volume socket; the dev attaches from any number of
terminals.
...
per-session flags work normally — for example, start an isolated worktree session, or
resume a prior one:
...
**Session lifecycle.** The processes run in the tmux server inside the agent container,
...
- The server and all its sessions live until the agent container stops or you
  `tmux -S … kill-server`. Have the container entrypoint respawn the primary session so
  there's always one to attach to.
...
short-lived (≤1h), scoped — to a `/creds` shelf that every consumer mounts read-only.
In this baseline all consumers read the same shelved secrets: the agent and the dev get
identical vended creds. The consumer side reads them through the resolver and adapters
described in [SECRETS.md](./SECRETS.md).

- A consumer has no signing capability and no upstream authority — it can only read
  what's on the shelf; it can't mint or widen anything.
- What's vended is decided in the sidecar (which roles / installations / repos / perms),
  in config baked into the sidecar image — a consumer can't change its scope.
- **Accepted residual:** a consumer fully controls the creds it can read (use + network
  exfil), and the shelf is at rest, readable by any process in a consumer that mounts
  it. The guarantee is blast radius (scoped + ≤1h), not secrecy.

**Per-consumer policy (the broker variant).** Giving the agent a narrower, audited, or
shorter-TTL grant than the dev — and removing the at-rest exposure — is the broker: a
per-consumer socket backed by a per-consumer grant table. That's a separate design, not
yet the shipping baseline — see [CREDENTIAL-BROKER.md](./CREDENTIAL-BROKER.md).

---

## 11. Layer 3 · Why still vend creds to the *dev* container

The agent is isolated, but the dev container still runs untrusted code — `npm/pip
install` postinstall scripts, and VS Code extensions. So keeping real creds out of it
is still worth it:
...
- **Vending** → a dep/extension gets only a scoped ≤1h token; the upstream authority
  stays in the sidecar, unmounted and unreachable. Small, and expiring.

Never co-locate the vending sidecar with untrusted code — that returns the upstream
authority to within reach of the dep/extension code and defeats the boundary. (Dropping
dev-side vending is a legitimate *risk acceptance*, not a free simplification — if you
do it, add `--ignore-scripts`, vetted extensions, and an egress allowlist.)

---

## 12. Human discipline (the patterns depend on these)

The architecture confines credentials; it does not stop you running code an agent wrote
or approving a prompt it induced. Those gaps are on you:

- **Agents prepare, humans review and apply.** Review diffs before running agent output
  in any trusted context, and before any hard-to-reverse or outward-facing action
  (deploy, merge, push, destroy). This closes nothing technically — it's the only
  control for risk #15.
- **Review `.devcontainer/`, `.vscode/settings.json`, and `devcontainer.json` diffs
  before a window *reload* and before a rebuild** — they're writable by code in the dev
  container, and some apply on reload (the `remote.extensionKind` RCE path, section 7).
- **Don't authorize the VS Code GitHub *git* sign-in** ("Publish to GitHub") — it arms
  the askpass-socket leak (section 5). Settings Sync and Copilot are fine.
- **Scope deliberately, least-privilege** — vend the narrowest grant a task needs (the
  *which-scope* half of risk #16).
...
- ♾️ **Can**, *if* the VS Code git OAuth session is authorized in the dev container,
...
- ♾️ **Can**, from a dev container with a desktop client attached, achieve **RCE on the
  ...
  going headless / web-client / VSCodium (section 7).
...
1. Agent container mounts **only** `/workspace` (+ the read-only `/creds` shelf, if
   vending) — none of the dev's own secrets (section 8).
2. The sidecar's upstream authority is mounted into **no** consumer; its vend config is
   **baked into the image**, not bind-mounted (section 10).
3. The `/creds` shelf is mounted **read-only** into consumers — they can't tamper with
   or forge vended creds (section 10). *(The broker variant instead mounts one socket per
   consumer — [CREDENTIAL-BROKER.md](./CREDENTIAL-BROKER.md).)*
4. `cap_drop: ALL` + `no-new-privileges` on every consumer; uid set by runtime `user:`,
   ...
   `CAP_SYS_PTRACE`, `network_mode/ipc: host`, or a Docker socket (sections 5 and 6).
5. VS Code: `git.useIntegratedAskPass:false`,
   ...
   `remoteEnv` **and** scrub them in interactive shells; keep the IPC-socket reaper
   (section 5).
...
7. Never co-locate the vending sidecar with untrusted code (section 11).
...
- `extensions/git/src/askpass.ts` — the askpass IPC server has **no caller auth**
  (it trusts the uid, not the caller).
